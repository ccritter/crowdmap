  ...
  reassignRoles(mapping, isActive) {
    ...
    if (isActive) {
      ...
      if (numActiveAddresses > 0) {
        ...
        console.log('test', this.unassignedSockets)
...
class Mapping {
  ...
  addToCrowd(socket) {
    ...
    socket.send({ address: '/' + this.address, args: [{type: 'i', value: this.source }]}); // TODO Socket Error TypeError [ERR_INVALID_ARG_TYPE]: The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type object
