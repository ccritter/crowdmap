...
class Client {
  ...
  configure(socket, config) {
    ...
    for (let entry of config) {
      try {
        ...
        this.state.setAddress(entry.address.substring(1), agg); // , source); TODO
        this.audience.addObserver(agg);
      } catch (e) {
        // TODO Send to client that the config was bad.
        console.log(e);
      }
    }
    ...
    this.socket.on('message', (msg, timeTag, info) => {
      if (msg.address === '/goodbye') {
        ...
        // TODO set an address as active, partition and assign the audience.
  ...
  removeAudienceMember(socket) {
    this.audience.removeMember(socket.id);
  }
