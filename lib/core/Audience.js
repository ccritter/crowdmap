...
class Audience {
  ...
  addMember(socket) {
    ...
    // TODO Add it to an assignment
    let assignments = Object.values(this.assignments);
    if (assignments.length > 0) {
      let smallestGroup;
      assignments.forEach(ass => {
        if (!smallestGroup || ass.length < smallestGroup.length) {
          smallestGroup = ass;
        }
      });
      smallestGroup.push(socket);
    } else {
